# Samrat-Yantra-
This is a working scale model of the Samrat Yantra, cut specifically for Ujjain. The Samrat Yantra is a sundial in which the gnomon's sloping edge is set parallel to Earth's axis, so it points at the celestial pole.
The two flanking dials are sections of a cylinder centred on that same edge, so the hour lines are straight and evenly spaced. The west dial reads the morning, the east dial the afternoon, and each is graduated by the shadow of the gnomon face it faces. 

The instrument gives local apparent solar time. To convert:

IST = dial reading − equation of time + 27 minutes
